const DEFAULT_QUICK_ALERT_OPTIONS = {
    title: '',
    subtitle: '',
    message: '',
    footer: '',
...
class QuickAlertController {
    ...
    static modalQueue = [];
    static inertedElements = new WeakMap();
    static idCounter = 0;
    ...
    activateRecord(doc, root, options, previousFocus, resolve) {
    ...
    render(doc, options, previousFocus, resolve) {
    ...
    renderCloseButton(doc, card, options) {
        if (!options.showCloseButton) {
            return;
        }
        const button = doc.createElement('button');
        button.className = this.classNames('quick-alert-close', this.customClassFor(options, 'closeButton'), options.closeButtonClass);
        button.type = 'button';
    ...
    renderContent(doc, card, options, titleId, messageId) {
    ...
    appendContent(doc, container, id, content) {
    ...
    renderIcon(doc, options) {
        const icon = doc.createElement('span');
        icon.className = this.classNames('quick-alert-icon', `quick-alert-icon-${options.type}`, this.customClassFor(options, 'icon'));
    ...
    renderActions(doc, card, options) {
    ...
    actionButton(doc, options, action, label) {
        const button = doc.createElement('button');
        const variant = this.variantForAction(options, action);
        button.className = this.classNames('quick-alert-button', `quick-alert-button-${action}`, `quick-alert-button-${variant}`, this.customClassFor(options, this.buttonClassSlot(action)), this.directButtonClass(options, action));
        button.type = 'button';
    ...
    async handleActionById(id, action) {
    ...
    resolveActions(options) {
        ...
        const actions = [options.showConfirmButton ? 'confirm' : 'ok'];
    ...
    ariaLabelForAction(options, action, fallback) {
        switch (action) {
            case 'confirm':
    ...
    variantForAction(options, action) {
        switch (action) {
            case 'confirm':
    ...
    directButtonClass(options, action) {
        switch (action) {
            case 'confirm':
    ...
    setLoading(record, action, loading) {
        ...
        record.card.querySelectorAll('[data-quick-alert-action]').forEach((button) => {
            button.disabled = loading;
        });
    }
    buttonClassSlot(action) {
        switch (action) {
            case 'confirm':
    ...
    startTimers(record) {
        ...
        if (record.options.showProgressBar) {
            ...
            progress.className = this.classNames('quick-alert-progress', this.customClassFor(record.options, 'progress'));
            const bar = record.card.ownerDocument.createElement('div');
            bar.className = this.classNames('quick-alert-progress-bar', this.customClassFor(record.options, 'progressBar'));
    ...
    finishById(id, action, dismissedBy) {
        const record = this.active.get(id);
        if (record) {
            this.finish(record, action, dismissedBy);
        }
    }
    finish(record, action, dismissedBy) {
    ...
    focusInitialAction(record) {
        if (record.options.toast) {
            return;
        }
        const button = record.card.querySelector('[data-quick-alert-action]');
        if (button) {
    ...
    trapFocus(event, card) {
        const focusable = Array.from(card.querySelectorAll('button, [href], input, select, textarea, [tabindex]:not([tabindex="-1"])')).filter((element) => !element.hasAttribute('disabled') && element.tabIndex !== -1);
        if (!focusable.length) {
            event.preventDefault();
    ...
    closeExistingById(id) {
        this.resolveQueuedById(id);
        this.finishActiveById(id);
    }
    resolveQueuedById(id) {
    ...
    static activeModalForDocument(doc) {
        for (const controller of QuickAlertController.controllers) {
            const modal = Array.from(controller.active.values()).find((record) => (!record.options.toast && record.card.ownerDocument === doc));
            if (modal) {
                return modal;
            }
        }
    ...
    ensureRoot(doc) {
        const existing = doc.querySelector('.quick-alert-root');
        if (existing) {
            return existing;
        }
    ...
    ensureToastStack(doc, root, position) {
    ...
    removeRootIfEmpty(doc) {
        const root = doc.querySelector('.quick-alert-root');
        root?.querySelectorAll('.quick-alert-toast-stack').forEach((stack) => {
    ...
    cardClass(doc, options) {
        const resolvedTheme = this.resolveTheme(doc, options.theme);
        return this.classNames('quick-alert-card', options.toast ? 'quick-alert-toast' : 'quick-alert-modal', `quick-alert-${options.position}`, `quick-alert-${options.transition}`, options.theme === 'auto' && 'quick-alert-theme-auto', `quick-alert-theme-${resolvedTheme}`, options.theme !== 'auto' && options.theme !== resolvedTheme && `quick-alert-theme-${options.theme}`, this.customClassFor(options, 'popup'));
    }
    resolveTheme(doc, theme) {
        if (theme !== 'auto') {
            return theme;
        }
        return doc.defaultView?.matchMedia?.('(prefers-color-scheme: dark)').matches ? 'dark' : 'default';
    }
    customClassFor(options, slot) {
...
const createQuickAlert = (defaults = {}) => new QuickAlertController(defaults);
const quickAlert = createQuickAlert();
...
//# sourceMappingURL=quick-alert.mjs.map
